...
def fetch(source):
    ...
    if not parsed.entries:
        ...
        reason = parsed.get("bozo_exception") or "源里没有任何条目"
        detail = f"HTTP {status}" if status else "无状态码"
        if ctype:
            detail += f", {ctype.split(';')[0]}"
        print(f"  [失败] {name}: {detail} — {reason}", file=sys.stderr)
        return []
...
def main():
    ...
    ap.add_argument("--strip-title-suffix", action="store_true",
                    help="去掉标题末尾的 “ - 媒体名”（Google News 会自动加上）")
    ...
    if args.strip_title_suffix:
        for item in items:
            item["title"] = re.sub(r"\s+[-–—]\s+[^-–—]{2,20}$", "", item["title"])
